...
type SignupStatusFilter = "all" | "pending" | "invited" | "canceled";

export function AdminEarlySignupsPage() {
  const { data: signups, isLoading } = useEarlySignups();
  const [sendingIds, setSendingIds] = useState<Set<string>>(new Set());
  const [cancellingIds, setCancellingIds] = useState<Set<string>>(new Set());
  const [statusFilter, setStatusFilter] = useState<SignupStatusFilter>("all");

  const filteredSignups = signups.filter((signup) => {
    if (statusFilter === "pending") {
      return !signup.confirmedAt && !signup.unsubscribedAt;
    }

    if (statusFilter === "invited") {
      return Boolean(signup.confirmedAt) && !signup.unsubscribedAt;
    }

    if (statusFilter === "canceled") {
      return Boolean(signup.unsubscribedAt);
    }
        ...
        <div className="mt-12 flow-root">
          <div className="mb-4 flex flex-wrap items-center gap-2 sm:px-6 lg:px-8">
            ...
            {(
              ...
              return (
                <button
                  className={`inline-flex cursor-pointer items-center rounded-md px-3 py-1.5 text-xs font-semibold ring-1 transition ring-inset ${
                    isActive
                      ? "bg-blue-600 text-white ring-blue-600"
                      : "bg-white text-gray-700 ring-gray-300 hover:bg-gray-50 dark:bg-gray-900 dark:text-gray-200 dark:ring-gray-700 dark:hover:bg-gray-800"
                  }`}
                  key={value}
                  type="button"
          ...
          <div className="custom-scrollbar overflow-x-auto sm:-mx-6 lg:-mx-8">
            <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
              {filteredSignups.length > 0 ? (
                ...
                <p className="rounded-md border border-dashed border-gray-300 px-4 py-6 text-sm text-gray-600 dark:border-gray-700 dark:text-gray-300">
                  No signups match this status.
                </p>
              )}
            </div>
          </div>
