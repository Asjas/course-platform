...
export const statsRouter = router({
  ...
  getSupportStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;
  ...
  // Get user statistics
  getUserStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;
  ...
  getCouponStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;
  ...
  getTeamLicenseStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;

    const [err, stats] = await fastify.to(fastify.cache.getTeamLicenseStats());
  ...
  getProgressStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;
  ...
  getWishlistStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;
  ...
  getAnnouncementStats: publicProcedure.use(isAdmin).query(async ({ ctx }) => {
    const fastify = ctx.reply.server;

    const [err, stats] = await fastify.to(fastify.cache.getAnnouncementStats());
